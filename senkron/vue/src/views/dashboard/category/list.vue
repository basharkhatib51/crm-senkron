    ...
    <b-row>
      ...
      >
        <b-button
          ...
          @click="$router.push({name:'category.trashed'})"
        >
          <font-awesome-icon
            :icon="['fas', 'trash']"
          />
    ...
    >
      ...
      >
        ...
        >
          ...
          <template #cell(children_categories_count)="data">
            <b-badge
              pill
              :variant="data.item.children_categories.length>0?'light-success':'light-warning'"
            >
              {{ data.item.children_categories.length }}
            </b-badge>
          </template>
          <template #cell(parent_category)="data">
            <b-badge
              pill
              :variant="data.item.parent_category?'light-success':'light-warning'"
            >
              {{ data.item.parent_category?data.item.parent_category.name:'parent' }}
            </b-badge>
          </template>
          ...
          <template #cell(options)="data">
            <div class="w-max">
              <router-link
                class="text-success"
                :to="{name:'category.edit',params: { category: data.item.id }}"
              >
                <font-awesome-icon
                  :icon="['fas', 'edit']"
                />
...
export default {
  ...
  computed: {
    fields() {
      return [
        ...
        { key: 'parent_category', label: this.$t('parent_category') },
        { key: 'children_categories_count', label: this.$t('children_categories_count') },
  ...
  methods: {
    ...
    ConfirmDelete() {
      this.$store.dispatch('category/Delete', this.element)
    },
  },
}
</script>
