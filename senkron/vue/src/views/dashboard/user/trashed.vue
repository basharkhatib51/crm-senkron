<template>
  <div>
    <b-modal
      ...
      @close="restoreUser=false"
      @cancel="restoreUser=false"
      @ok="ConfirmRestore"
    >
      <b-badge
        class="mt-1"
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['user/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('user/GetTrashed')
  },
  methods: {
    RestoreUser(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('user/Restore', this.user)
    },
  },
}
</script>
