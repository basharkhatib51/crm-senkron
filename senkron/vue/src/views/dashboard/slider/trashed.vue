<template>
  <div>
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Slider') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
      {{ $t('Sliders') }}
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'slider.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
          ...
          <template #cell(options)="data">
            <div class="w-max">
              <a
                ...
                @click="RestoreSlider(data.item.id)"
              >
                <font-awesome-icon
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['slider/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('slider/GetTrashed')
  },
  methods: {
    RestoreSlider(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('slider/Restore', this.element)
    },
  },
}
</script>
