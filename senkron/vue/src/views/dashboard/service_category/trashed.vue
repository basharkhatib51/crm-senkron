<template>
  <div>
    <b-modal
      ...
      :title="$t('Restore Service Category')"
      :ok-title="$t('Restore')"
    ...
    >
      ...
      >
        {{ $t('are you sure you want to restore this Service Category') }}
      </b-badge>
    </b-modal>
    <h3 class="mb-3">
    ...
    <b-row>
      <b-col cols="12">
        <b-button
          ...
          @click="$router.push({name:'service_category.list'})"
        >
          <font-awesome-icon
            :icon="['fas', 'arrow-left']"
          />
          ...
          <template #cell(options)="data">
            <div class="w-max">
              <a
                ...
                @click="RestoreCategory(data.item.id)"
              >
                <font-awesome-icon
...
export default {
  ...
  computed: {
    ...
    items() {
      return this.$store.getters['serviceCategory/Trashed']
    },
    totalRows() {
  ...
  created() {
    this.$store.dispatch('serviceCategory/GetTrashed')
  },
  methods: {
    RestoreCategory(val) {
    ...
    ConfirmRestore() {
      this.$store.dispatch('serviceCategory/Restore', this.element)
    },
  },
}
</script>
